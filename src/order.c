...
char   * order_fname =  "order.slc";        /* 订单文件名                          */
...
BOOL similar_order(order_struct * order1, order_struct * order2)
{
    ...
    if(order1->CUTS != order2->CUTS)
        return fail;
    if(order1->YX   != order2->YX)
        return fail;
    if(order1->TRIM != order2->TRIM)
        return fail;
    if(STRCMP(order1->FLUTE, order2->FLUTE))
        return fail;
    if(STRCMP(order1->SPECCUT, order2->SPECCUT))
        return fail;
